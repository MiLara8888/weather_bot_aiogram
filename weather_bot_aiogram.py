...
async def operat1(message):
    ...
    if res['cod'] == '404':
        ...
        sunrise = 'Восход солнца - {}'.format(time.strftime('%X', time.localtime(int(res['sys']['sunrise'])+10800)))
        sunset = 'Заход солнца - {}'.format(time.strftime('%X', time.localtime(int(res['sys']['sunset'])+10800)))
